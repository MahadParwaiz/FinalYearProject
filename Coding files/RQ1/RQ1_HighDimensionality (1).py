...
def generate_non_linear_friedman_data(n_samples, n_features,
                                      noise=0.0,
                                      skew_factor=1.0,
                                      seed=42):
    """
    Generates a modified Friedman-style dataset using only non-linear terms.
    True informative features: x0, x1, and x2.
    
    The target is computed as:
        y = 10 * sin(pi * x0 * x1) + 20 * (x2 - 0.5)**2 + noise
    
    All other features (if n_features > 3) are random noise.
    """
    np.random.seed(seed)
    # Generate features uniformly between 0 and 1.
    X = np.random.rand(n_samples, n_features)
    
    # Compute y using only the non-linear terms.
    y = 10 * np.sin(np.pi * X[:, 0] * X[:, 1]) + 20 * (X[:, 2] - 0.5)**2
    y += np.random.normal(0, noise, size=n_samples)
    
    # Apply skew transformation if requested.
    if skew_factor != 1.0:
        shift = abs(min(y)) + 1e-3
        y = (y + shift) ** skew_factor

    # Calculate a naive "imbalance ratio" (as in your original function).
...
def evaluate_regression_model(model, X_train, X_test, y_train, y_test, seed=42):
    ...
    top_10 = sorted_idx[:10]   # ← define top_10 here
...
if __name__ == "__main__":
    ...
    sample_sizes = [40, 100]
    feature_list = [10, 30, 50, 100, 300, 500, 1000]
    ...
    boxplot_metric(df_all, 'Recovery of Causative Features in Top 5')
        ...
        # 1. Define which columns uniquely identify each experimental cell:
    group_cols = [
        'SampleSize',
        'NumFeatures',
        'Noise',
    ...
    # 2. Aggregate to get median and std of RMSE_Test and Recovery across seeds:
    summary_df = df_all.groupby(group_cols, as_index=False).agg({
        'RMSE_Test': ['median', 'std'],
        'Recovery of Causative Features in Top 5': ['median', 'std'],
        'Train_Time':                       ['median', 'std']
    })

    # 3. Flatten the MultiIndex columns:
    #    e.g. turn ('RMSE_Test','median') → 'RMSE_Test_median', etc.
    summary_df.columns = [
        '_'.join(col).rstrip('_')  # join tuples, drop trailing underscore if any
        for col in summary_df.columns.values
    ]

    # 4. Write to CSV:
    summary_df.to_csv('rmse_and_recovery_summary.csv', index=False)
    print("Wrote summary to rmse_and_recovery_summary.csv")
