...
for corr in correlations:
    for seed in seeds:
        ...
        Y = (
            1.5 * (np.sin(2 * X[:, 0])**2)             # sine term on feature 1
          + 1.5 * (np.sin(2 * X[:, 1])**2)           # sine term on feature 2
          + 0.5 * (X[:, 0])**2                      # quadratic on feature 1
          + 0.5 * (X[:, 1])**2)                   # quadratic on feature 2
        ...
        start_mlp = time.time()
        mlp = MLPRegressor(random_state=seed, max_iter=10000)
        ...
        mlp_f1 = MLPRegressor(random_state=seed, max_iter=10000)
        ...
        mlp_f2 = MLPRegressor(random_state=seed, max_iter=10000)
...
df.to_csv("informative-informative_correlation.csv", index=False)
# --------------------------------------------------------------------
# Seaborn Settings
# --------------------------------------------------------------------
...
g.fig.savefig("1_PermutationImportance.png")
plt.close()
...
plt.tight_layout()
plt.savefig("ConditionNumber.png")
plt.close()
# --------------------------------------
